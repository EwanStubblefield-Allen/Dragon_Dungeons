<template>
  <section class="row">
    <div class="col-12 col-md-3 col-lg-2 overflow-auto infoBar">
      <section class="row p-3">
        ...
        <div v-for="(l, index) in list" :key="l" class="col-12 col-sm-6 col-md-12">
          <router-link :to="{ name: 'Campaign', params: { campaignId: l.toLowerCase().replaceAll(' ', '-') } }" v-if="camPage >= index">
    ...
    <div class="col-12 col-md-9 col-lg-10 offset-md-3 offset-lg-2">
      <div v-if="route.params.campaignId == 'basics'">
        <CampaignBasics />
      </div>

      <div v-else-if="route.params.campaignId == 'notes'">
        <CampaignNotes />
      </div>

      <div v-else-if="route.params.campaignId == 'encounters'">
        <CampaignEncounters />
      </div>

      <div v-else-if="route.params.campaignId == 'creatures'">
        <CampaignCreatures />
      </div>
    </div>
  </section>
...
import CampaignCreatures from '../components/CampaignCreatures.vue'
...
export default {
  setup() {
    ...
    watchEffect(() => {
      ...
      for (let i = list.length - 1; i > camPage; i--) {
        if (list[i].toLowerCase().replace(' ', '-') == route.params.campaignId && AppState.tempCharacter) {
          router.push({ name: 'Campaign', params: { campaignId: list[camPage].toLowerCase().replace(' ', '-') } })
        }
      }
    })
    return {
  ...
  components: { CampaignBasics, CampaignNotes, CampaignEncounters, CampaignCreatures }
}
</script>
